...
def parseTags(toParse: str, parsedTags: list) -> list:
    ...
    if toParse.find(" ") < firstEndTag and toParse[0] == "<":
        tag = toParse[1:toParse.find(" ")]
        parsedTags.append(tag)
        toParse = toParse[firstEndTag+1:]
    else:
        toParse = "" # TODO: deal with different formatting on a given line.
...
def main(args: list):
    ...
    for each in links:
        try: # TODO: create formatted output for when there is no README.(may need a second test repo for related UTs.)
        ...
        except selenium.common.exceptions.StaleElementReferenceException: # This is an attempt to get around stale element issues by loading the necessary cookies and trying again.
            if "title=\"README.md\"" in each.get_attribute("outerHTML"):
    ...
    tags = page.page_source[page.page_source.find("<article class=\"markdown-body entry-content container-lg\""):page.page_source.find("</article>")+10].split("\n") # Cut the page source down to just the article tags surrounding the README text.
    print(tags)
    # print(text)
                                    # ----- Parse Tags ----- #
